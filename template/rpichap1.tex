...
While this inspirational quote may hold true for the human character, it does not, unfortunately, hold true for scientific datasets.
With changing technology, data collected by researchers grew at an astounding rate with new technology.
...
A project at CERN reports that it generates on the order of four thousand new datasets per day \cite{Branco2008}.
This explosion of data marks a change in the perspective of research from a software driven approach, where programs are used to confirm results using data, to a data driven approach, where programs are used to extract trends and conclusions from existing data.
The proliferation of powerful and mobile computing devices has allowed researchers to generate data faster than it can be tracked \cite{Bose:2005:LRS:1057977.1057978}.
...
Highlights a shift from software driven research to data driven research, meaning that versioning techniques must also shift from a software focused method to a data focused method.

\section{Types of data sets}
I/O Block level versioning \cite{Flouris04clotho:transparent}
