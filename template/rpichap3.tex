...
The first approach, seen in Figure \ref{DiscardedFig}, simply extends the \gls{provenance} relation with additional concepts to capture more types of relationships.
\begin{figure}
	\centering
...
Version 1 also relates to altered \glspl{attribute} since it provides the \textit{pre-value} used to contextualize Version 2's \textit{post-value}.
...
Adding the \glspl{attribute} as concepts to the model addresses PROV's and PAV's flat approach to \gls{version} relations, but the \glspl{attribute} do not capture the inter-relation between objects for \textit{New} and \textit{Old} \glspl{attribute}.
...
The \textit{Old} \glspl{attribute} should not exists within Version 2.
Associating \textit{Old} \glspl{attribute} with Version 1 would be more appropriate and intuitive to understand.
The model does not capture the \underline{type} of change, making the result a listing of \glspl{attribute} without the \gls{version} differences to contextualize the relationship between the \glspl{version}.

From a different direction, Figure \ref{DiscardedFig2} shows a model created by starting from the \gls{log}.
\begin{figure}
	\centering
	\vspace{0.0in} % normally the command here would be \includegraphics
	%	\includegraphics{figures/Addition.png}
...
\Glspl{log} often break down and group changes by \glspl{attribute}.
...
The primary drawback of the log-based construction is that the \gls{log} assumes all responsibility for every modification to the data even though the document only reports the differences.
The \gls{version} objects are also left out of the model, leaving the log concept in possession of the attributes.
One of the major breakthroughs with the log-centric model construction is that while specific values are kept in the log, pre- and post- values do not need to be in the model.
...
Pre- and post- values do not appear in the model because the presence and kind of change is more valuable information than assessing the explicit values involved.
As a data set becomes more volatile, more values would need to be stored, resulting in more copying of the data rather than summarization of the changes.
Notice in Figure \ref{DiscardedFig3} that \textit{Attribute} has now become disconnected from either \gls{version}.
...
The larger issue with both the log-based and hybrid approach is that the model resembles a tree more than a graph, making \gls{linked} queries less powerful, as most of the concepts are disconnected.

A fourth formulation, in Figure \ref{DiscardedFig4}, leverages the insight that when a change interacts with an \gls{attribute}, the \gls{attribute} is different in the next \gls{version}.
\begin{figure}[b]
	\centering
	\vspace{0.0in} % normally the command here would be \includegraphics
	%	\includegraphics{figures/Addition.png}
...
\end{figure}
The model addresses the attribution problem by forming two \glspl{attribute}, each associated with a different \gls{version}.
These \glspl{attribute} inform a change which acts upon both \textit{Version} concepts.
...
A new attribute would not exist in \textit{Version 1} so \textit{Attribute 1} and its associated properties (arrows) are removed, leaving a unique construction implying an \gls{attribute} \gls{add}.
...
\section{Model Objects} \label{sec:model}
...
The model bases \glspl{change} around the three core versioning operations because their commonality across systems provides a fundamental basis for comparisons.
\glspl{add} occur when an \gls{attribute} appears only in the \textbf{right-hand version}.
When an \gls{attribute} only shows up in the \textbf{left-hand version}, the model captures the relationship as an \gls{invalidate}.
