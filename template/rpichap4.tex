...
The change log generating script standardizes the comparison process to encompass every entry in the \mintinline{python}{generate_logs} function.
As seen in lines 7 through 61 in Appendix \ref{app:clpx}, the script can be modularized to handle different shapefile structure while still using the same \mintinline{python}{generate_logs} function.
...
In the \mintinline{python}{write_removed} function on line 184, the text consumption of each data format can be compared by looking at the text written in each mode.
